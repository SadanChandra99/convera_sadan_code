...
public final class DashboardHelper {
    ...
    public static DashboardPreference getValidDashboard(Integer userId) {
    	Widget w1 = new Widget(1L, "payment widget", "template-1", "localhost:8080", false, LocalDateTime.now(), LocalDateTime.now(), 31);
    	Widget w2 = new Widget(1L, "market news widget", "template-2", "localhost:8984", true,  LocalDateTime.now(), LocalDateTime.now(), 33);
    	Widget w3 = new Widget(1L, "quotes widget", "template-3", "localhost:4820", true,  LocalDateTime.now(), LocalDateTime.now(),45);
